...
void printMemory(void *start, bool onlyList) {
	...
	while (current) {
		if (current->is_free) {
			std::cout << "|F:" << (current->total_size - sizeof(*current));
		} else {
			std::cout << "|U:" << (current->total_size - sizeof(*current));
		}
		size += (current->total_size - sizeof(*current));
		blocks++;
...
void updateStats(void *start, stats &current_stats, size_t bytes_mmap, int blocks_mmap) {
	...
	while (current) {
		...
		current_stats.num_allocated_bytes += (current->total_size - sizeof(*current));
		current_stats.num_allocated_blocks++;
		if (current->is_free) {
			current_stats.num_free_blocks++;
			current_stats.num_free_bytes += (current->total_size - sizeof(*current));
		}
		current = current->next;
	}
